...
Build **`zen-install.exe`** from the repo (Node ≥ 22; `esbuild` and `postject` are
devDependencies) and run it:

```bat
npm run build-installer     :: produces dist-zen\zen-install.exe
...
> There is no GitHub release / hosted download — the exe is built locally.

## Autostart the RSS server

Zen's live folders only sync while the RSS server (`dist/index.js`) is running.
`scripts/keep_alive.cjs` is a keep-alive wrapper: it spawns the server and restarts it
if it exits, appending logs to `server.log`. Pick one way to launch it:

**Startup folder (per user, simplest)** — put a shortcut to
`scripts/start_hidden.ps1` (starts `node` headless via WMI) or `start.bat` (cmd) in
`shell:startup`. Both scripts contain hardcoded paths for this repo — edit them to
match your checkout before use.

**Task Scheduler (runs at log on, survives reboot)** —

```bat
schtasks /create /tn "RSS Server" /tr "powershell.exe -ExecutionPolicy Bypass -File \"<repo>\scripts\start_hidden.ps1\"" /sc onlogon /f
```

or add it via `taskschd.msc` (trigger: **At log on**; action: start the script). No
admin rights are needed — the server listens on `http://localhost:3000`, a port above
1024.

**Manually** — run `node scripts/keep_alive.cjs` (or `npm start`) in a terminal and
leave it open.

Once the server is up, open Zen — live folders appear within seconds.
