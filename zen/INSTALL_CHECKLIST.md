...
## 0. Autostart the RSS server (optional but recommended)

The server must be running whenever Zen syncs. `scripts/keep_alive.cjs` starts
`dist/index.js` and restarts it if it exits (logs to `server.log`). Set it up to
launch at login so you don't have to think about it:

- [ ] **Startup folder:** put a shortcut to `scripts/start_hidden.ps1` or `start.bat` in
      `shell:startup` (edit the hardcoded paths inside the script to match your checkout).
- [ ] **Or Task Scheduler:** `schtasks /create /tn "RSS Server" /tr "powershell.exe
      -ExecutionPolicy Bypass -File \"<repo>\scripts\start_hidden.ps1\"" /sc onlogon /f`
      (or add it in `taskschd.msc` with trigger **At log on**).
- [ ] **Or manually:** `node scripts/keep_alive.cjs` in a terminal you keep open.
- [ ] Verify: `curl http://localhost:3000/health` returns `{"status":"OK",...}` after a
      reboot without you starting anything.
